...
const css = `
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .seed-container.hidden { opacity: 0; pointer-events: none; }

  .seed {
    width: 3vmin; height: 4vmin;
    min-width: 12px; min-height: 16px;
    background: radial-gradient(ellipse at 40% 30%, #8B6914, #5a3e0a);
    border-radius: 50% 50% 50% 50% / 60% 60% 40% 40%;
    box-shadow: 0 0 6px rgba(255,200,50,0.3), inset 0 1px 2px rgba(255,220,100,0.4);
    position: relative;
  ...
  .seed::after {
    ...
    bottom: -6px; left: 50%;
    transform: translateX(-50%);
    width: 1.5vmin; height: 4vmin;
    min-width: 5px; min-height: 14px;
    background: linear-gradient(to bottom, #5a3e0a, transparent);
    border-radius: 0 0 50% 50%;
  }

  @keyframes seed-pulse {
    0%, 100% { transform: scale(1); box-shadow: 0 0 6px rgba(255,200,50,0.3); }
    50% { transform: scale(1.08); box-shadow: 0 0 14px rgba(255,200,50,0.6); }
  }
  ...
  .message-overlay.visible { opacity: 1; pointer-events: none; }
  ...
  .flower__white-circle::after {
    ...
    background-image:
      repeating-linear-gradient(135deg,rgba(0,0,0,.03) 0,rgba(0,0,0,.03) 1px,transparent 1px,transparent 12px),
      repeating-linear-gradient(45deg,rgba(0,0,0,.03) 0,rgba(0,0,0,.03) 1px,transparent 1px,transparent 12px),
  ...
  .flower__grass__overlay {
    ...
    filter: blur(1.5vmin); z-index: 100;
  }
  ...
  .flower__g-front__leaf-wrapper:nth-child(even) { left:0;transform:rotateY(-180deg) rotate(5deg);animation:flower__g-front__leaf-left-ans 1s ease-in backwards; }
  ...
  .flower__g-front__leaf {
    ...
    background-image:linear-gradient(to bottom left,transparent,#000),linear-gradient(to bottom right,#159faa 50%,transparent 50%);
  ...
  /* g-fr */
  .flower__g-fr {
    position:absolute;bottom:-4vmin;left:0;transform-origin:bottom left;z-index:10;
  ...
  .flower__g-fr__leaf {
    position:absolute;top:0;left:0;width:10vmin;height:10vmin;
    border-radius:100% 0% 0% 100%/100% 100% 0% 0%;
    box-shadow:inset 0 2px 1vmin rgba(44,238,252,0.2);
    background-image:linear-gradient(to bottom left,transparent,#000 98%),linear-gradient(to bottom right,#23f0ff 45%,transparent 50%);
  ...
  /* paused state – holds all animations until we set bloomed */
  ...
  .ripple {
    position: fixed;
    border-radius: 50%;
    background: rgba(107, 240, 255, 0.25);
    transform: scale(0);
...
function Flower({ num, children }) {
  return (
    <div className={`flower flower--${num}`}>
...
export default function MagicalFlower() {
  ...
  return (
    <div
      ref={appRef}
      className={`flower-app${bloomed ? "" : " paused"}`}
    ...
    >
      ...
      {/* Seed — hidden after bloom */}
      <div className={`seed-container${bloomed ? " hidden" : ""}`}>
        <div className="seed" />
        <div className="hint-text">touch me to bloom ✦</div>
      </div>

      {/* Full flower garden */}
      <div className={`flowers${bloomed ? " bloomed" : ""}`}>

        <Flower num={1}>
          {[1,2,3,4,5,6].map(i => <div key={i} className={`flower__line__leaf flower__line__leaf--${i}`} />)}
        </Flower>

        <Flower num={2}>
          {[1,2,3,4].map(i => <div key={i} className={`flower__line__leaf flower__line__leaf--${i}`} />)}
        </Flower>
        ...
        <div className="growing-grass">
          <div className="flower__grass flower__grass--1">
            ...
            {[1,2,3,4,5,6,7,8].map(i => <div key={i} className={`flower__grass__leaf flower__grass__leaf--${i}`} />)}
        ...
        <div className="grow-ans" style={{"--d":"3.2s"}}>
          <div className="flower__g-fr">
            <div className="leaf" />
            {[1,2,3,4,5,6,7,8].map(i => <div key={i} className={`flower__g-fr__leaf flower__g-fr__leaf--${i}`} />)}
          </div>
        </div>

        {[
          { n:0, delays:["3s","2.2s","3.4s","3.6s"] },
          { n:1, delays:["3.6s","3.8s","4s","4.2s"] },
          { n:2, delays:["4s","4.2s","4.4s","4.6s"] },
          { n:3, delays:["4s","4.2s","3s","3.6s"] },
          { n:4, delays:["4s","4.2s","3s","3.6s"] },
          { n:5, delays:["4s","4.2s","3s","3.6s"] },
          { n:6, delays:["4.2s","4.4s","4.6s","4.8s"] },
          { n:7, delays:["3s","3.2s","3.5s","3.6s"] },
        ].map(({ n, delays }) => (
      ...
      {/* Love message — fades in after bloom */}
      <div className={`message-overlay${bloomed ? " visible" : ""}`}>
        <div className="message-box">
          <p>Flower for a pretty lady,<br /><span>walang flower kaya ito nalang.</span></p>
        </div>
      </div>
    </div>
  );
